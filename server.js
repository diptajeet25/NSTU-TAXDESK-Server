...
async function run() {
  try {
    ...
    app.get('/manage-user-stats',async(req,res)=>
    {
      ...
      const activeUsers=await usersCollection.countDocuments({active:true,role:'user'});
      const inactiveUsers=await usersCollection.countDocuments({active:false,role:'user'});
      console.log(`User Count: ${userCount}, Active Users: ${activeUsers}, Inactive Users: ${inactiveUsers}`);
      res.json({userCount, activeUsers, inactiveUsers});
